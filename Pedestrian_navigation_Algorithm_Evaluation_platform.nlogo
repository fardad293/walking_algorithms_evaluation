...
to LTG
  ...
  let edge1 []
  foreach vis [
    if any? ([neighbors] of ?) with [pcolor = brown] [set edge1 lput ? edge1]
  ]
  foreach edge1 [
...
to LTG2
  ...
  foreach ed [
    if count ([neighbors4] of ?) with [member? self ed] = 1 or count ([neighbors] of ?) with [member? self ed] = 1 [set ltg-list lput ? ltg-list]
  ]
end
